	...
	resetMovieAPI: function () {
 	   ...
 	   case 'ytsAPI':
           ...
           case 'tvAPI':
            ...
            case 'tmpLocation':
                ...
                value = path.join(field.val(), 'Phancast');
                break;
            case 'activateVpn':
                $('.vpn-connect').toggle();
                value = field.is(':checked');
		...
		remBrowStre: function () {
			if (process.platform == 'linux') {
				require('child_process').exec('gnome-terminal -x bash -c "echo \'This setting requires Admin Rights\'; echo; sudo echo; sudo echo \'Authentication Successful\'; sudo echo; sudo rm /usr/share/applications/phancast; echo; echo \'Done! Press any key to close ...\'; read" & disown');
			}
		},

		writeDesktopFile: function (cb) {
			var pctPath = process.execPath.substr(0,process.execPath.lastIndexOf("/")+1);
			var Exec = pctPath+'Phancast'; //process.execPath
			fs.writeFile(gui.App.dataPath+'/phancast', '[Desktop Entry]\nVersion=2.0\nName=Phancast Player\nComment=Phanacst Interactive downloads and streams torrents instantly, directly from your browser! Just click on the torrent or magnet link and start downloading and playing it easily and in no time.\nExec='+Exec+' %U\nPath='+pctPath+'\nIcon='+pctPath+'phancast.png\nTerminal=false\nType=Application\nMimeType=application/x-bittorrent;x-scheme-handler/magnet;video/avi;video/msvideo;video/x-msvideo;video/mp4;video/x-matroska;video/mpeg;\n', cb);      
		},


		//function to move phancast to /usr/share/applications
		//function to set phancast as default for magnet links on browser and phancast
        regMagnet: function () {
			if (process.platform == 'linux') {
				this.writeDesktopFile(function(err) {
					if (err) throw err;
					var desktopFile = gui.App.dataPath+'/phancast';
					//var desktopFile = '$HOME/.Phancast/phancast';
					var tempMime = 'x-scheme-handler/magnet';

					//xdg-mime and gvfs-mime configures defaults for applications
					require('child_process').exec('gnome-terminal -x bash -c "echo \'Streaming magnet links directly from your browser requires Admin Rights\'; echo; sudo echo; sudo echo \'Authentication Successful\'; sudo echo; sudo mv -f '+desktopFile+' /usr/share/applications; sudo xdg-mime default phancast '+tempMime+'; sudo gvfs-mime --set '+tempMime+' phancast; echo; echo \'Success! Press any key to close ...\'; read" & disown');
				});
		...
		regTorrent: function () {
			if (process.platform == 'linux') {
				...
				var pctPath = process.execPath.substr(0,process.execPath.lastIndexOf("/")+1)+"../../../../Resources/app.nw/";
				require('child_process').exec('"'+pctPath+'src/duti/duti" -s media.Phancast.player .torrent viewer');
				alert("Success!");
			} else {
				fs.writeFile(gui.App.dataPath+'\\register-torrent.reg', 'REGEDIT4\r\n[HKEY_CURRENT_USER\\Software\\Classes\\Phancast.player\\DefaultIcon]\r\n@="'+process.execPath.split("\\").join("\\\\")+'"\r\n[HKEY_CURRENT_USER\\Software\\Classes\\Phancast.player\\shell\\open\\command]\r\n@="\\"'+process.execPath.split("\\").join("\\\\")+'\\" \\"%1\\""\r\n[HKEY_CURRENT_USER\\Software\\Classes\\.torrent]\r\n@="Phancast.player"\r\n"Content Type"="application/x-bittorrent"', function (err) {
					if (err) throw err;
